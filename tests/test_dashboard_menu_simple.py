"""Simple tests for dashboard menu management endpoints"""
...
class TestDashboardMenuSimple:
    """Simple test suite for dashboard menu management endpoints"""
    ...
    @patch('app.api.v1.endpoints.dashboard_menu.get_menu_item_service')
    def test_get_restaurant_menu_items_success(
        self, mock_get_service, mock_get_current_user, client, sample_menu_item
    ):
        """Test successful retrieval of restaurant menu items"""
        # Mock the auth to return user and restaurant_id
        mock_user = {"id": "user-123", "email": "test@example.com"}
        mock_get_current_user.return_value = (mock_user, "restaurant-123")
    ...
    def test_create_menu_item_success(
        self, mock_get_service, mock_get_restaurant_id, client, sample_menu_item
    ):
    ...
    def test_delete_menu_item_success(
        self, mock_get_service, mock_get_restaurant_id, client
    ):
    ...
    def test_toggle_menu_item_availability_success(
        ...
        mock_service.toggle_availability.return_value = updated_item
        mock_get_service.return_value = mock_service
        ...
        mock_service.toggle_availability.assert_called_once_with(
            item_id="item-123",
            restaurant_id="restaurant-123",
            is_available=False
        )
    ...
    def test_get_menu_categories_success(
        ...
        mock_service.get_categories_for_restaurant.return_value = categories
        mock_get_service.return_value = mock_service
        
        headers = {"Authorization": "Bearer test-token"}
        response = client.get("/api/dashboard/menu/categories/list", headers=headers)
        
        assert response.status_code == 200
        data = response.json()
        assert data == categories
        
        mock_service.get_categories_for_restaurant.assert_called_once_with("restaurant-123")
